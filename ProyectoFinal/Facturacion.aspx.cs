...
namespace ProyectoFinal
{
    public partial class Facturacion : System.Web.UI.Page
    {
        ...
        protected void Bfacturar_Click(object sender, EventArgs e)
        {
            ...
            if (ClsFactura.AgregarMaestroFactura(tnombre.Text, LTOTAL.Text) > 0)
            {
                foreach (GridViewRow item in GridView1.Rows)
                {
                    ...
                    if (ClsFactura.AgregarDetalleFactura(ClsFactura.codigoDF, codigo, cantidad, precio) > 0)
                    {
                        ClsFactura.codigoDF++;
                    }
                }
        ...
        {
            tnombre.Text = ClsProductos.BuscarProducto(tnombre.Text, tcodigo.Text);
            tprecio.Text = Convert.ToString(ClsProductos.precio);
            tcantidad.Focus();
        }
    }
}